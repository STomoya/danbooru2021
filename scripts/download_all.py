...
def download(folder, output=TEMP):
    command = f'rsync -r rsync://176.9.41.242:873/danbooru2021/original/{folder}/ {output}'
...
def main():
    ...
    for intfolder in range(start, TOTAL_FOLDERS):
        folder = str(intfolder).zfill(4)
        download(folder, TEMP)
        downloaded_files = glob.glob(os.path.join(TEMP, '*'))
